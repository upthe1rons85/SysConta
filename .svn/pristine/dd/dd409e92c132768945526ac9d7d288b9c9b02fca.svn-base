Module MdNumLetras
    ...
    Private Sub Inicializar()       'Comprueba si ya se han inicializado los valores
        ...
        End If
    End Sub
    Public Function NumerosLetras(ByVal Importe As Double, ByVal FormatoTxt As VbStrConv) _
     As String
        ...
        If strTmp = "" Then
            NumerosLetras = StrConv("CERO PESOS" & Centavos(Importe), FormatoTxt)
        ElseIf strTmp = "UN" Then
            NumerosLetras = StrConv(strTmp & " PESO" & Centavos(Importe), FormatoTxt)
        ElseIf Right$(strTmp, 4) = "ONES" Or Right$(strTmp, 2) = "ON" Then
            NumerosLetras = StrConv(strTmp & " DE PESOS" & Centavos(Importe), FormatoTxt)
        Else : NumerosLetras = StrConv(strTmp & " PESOS" & Centavos(Importe), FormatoTxt)
        End If
    End Function
  Private Function ImporteLetras(ByVal Importe As Double) As String       
    ...
    Private Function Centavos(ByVal Importe As Double) As String
        ...
        Centavos = " " & Right$(Format$(Importe, "0.00"), 2) & "/100 m. n."
    End Function
